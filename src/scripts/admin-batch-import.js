(() => {
  const init = () => {
    ...
    document.querySelector('#batch-import-submit').addEventListener('click', async () => {
      ...
      try {
        const response = await fetch('/api/admin/students', { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify(payload), credentials: 'same-origin' });
      ...
      } catch (error) { result.textContent = `Unable to complete the import: ${error?.message || 'network error'}.`; }

(() => {
  function init() {
    ...
    const parseRows = (raw) => {
      ...
      const seen = new Set();
      return rows.filter((row) => !seen.has(row.email) && seen.add(row.email));
    };
      ...
      overlay.innerHTML = `
        ...
        <section class="batch-import-dialog" role="dialog" aria-modal="true" aria-labelledby="batch-import-title">
          ...
          <div class="batch-import-body">
            ...
            <label>Students<textarea id="batch-import-input" rows="10" placeholder="| First Name | Last Name | Email |\n| April-May | O'Meara | asaessentials1@gmail.com |"></textarea><span class="field-note">Markdown tables, tabs, or columns separated by multiple spaces are supported.</span></label>
            <div class="batch-import-preview"><div class="batch-import-count" id="batch-import-count">0 students detected</div><div class="batch-import-table-wrap"><table><thead><tr><th>Name</th><th>Email</th></tr></thead><tbody id="batch-import-preview-body"></tbody></table></div></div>
            ...
            <div id="batch-import-custom" hidden><label>Subject<input id="batch-import-subject" /></label><label>Message<textarea id="batch-import-body" rows="7"></textarea></label></div>
    ...
    const wireOverlay = () => {
      ...
      const subject = overlay.querySelector('#batch-import-subject');
      const body = overlay.querySelector('#batch-import-body');

      const syncClasses = () => {
        const source = document.querySelector('#manual-class');
        classSelect.innerHTML = source ? source.innerHTML : '<option value="">Select a class</option>';
      ...
      submit.addEventListener('click', async () => {
        ...
        if (action.value === 'custom' && (!subject.value.trim() || !body.value.trim())) {
        ...
        try {
          ...
          result.textContent = `Imported ${added} student${added === 1 ? '' : 's'}${failed ? `; ${failed} failed.` : '.'}`;
          document.querySelector('#student-class')?.dispatchEvent(new Event('change'));
        } catch (error) {
    ...
    trigger.addEventListener('click', () => createOverlay());
  }
